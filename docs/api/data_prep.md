...
at once. `ChunkSpool` takes batches as they are extracted and spools each
member to disk, assembling the `.npz` at the end (via `write_npz`) — peak drops
from ~2.5x the corpus to ~0.25x.
