"""
ConvLSTMBaseAttn: Baseline model with attention pooling over LSTM positions.

Replaces center-position selection with learned attention over all LSTM positions.
"""
...
class ConvLSTMBaseAttn(BaseModel):
    """
    Baseline model with learned attention pooling over LSTM positions.

    Same architecture as ConvLSTMBase but replaces center-position extraction
    with a learned attention mechanism that weights all LSTM positions.
