"""
ConvLSTMDwellLNAttn: Dwell model with LayerNorm + cross-attention.

Same architecture as ConvLSTMDwellBNAttn but uses LayerNorm (implemented as
GroupNorm with num_groups=1) instead of BatchNorm. LayerNorm normalizes per
sample across all channels, providing maximum batch independence.
"""
...
class ConvLSTMDwellLNAttn(BaseModel):
    """
    Dwell model with LayerNorm and cross-attention for learning motor-sensor offset.

    Same architecture as ConvLSTMDwellBNAttn but with LayerNorm instead of BatchNorm.
    ...
    def __init__(
        ...
        self.signal_branch = SignalBranch(
            conv_channels=conv_channels, norm_type="layernorm", in_channels=signal_in_channels
        )

        # Sequence branch (with LayerNorm)
        self.sequence_branch = SequenceBranch(
            in_channels=seq_in_channels, conv_channels=conv_channels, norm_type="layernorm"
        )

        # Feature branch (dwell + signal level features, with LayerNorm)
        self.feature_branch = FeatureBranch(
            num_features=num_features, conv_channels=conv_channels, norm_type="layernorm"
        )
