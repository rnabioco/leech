        ...
        with gzip.open(output.report, "wt") as out:
            ...
            for pod5_file in params.pod5_files:
                ...
                try:
                    ...
                    out.write(
                        f"{wildcards.sample}\t{pod5_path}\t{status}\t{num_reads}\t{file_size}\t{avg_len}\t{min_len}\t{max_len}\t{error_msg}\n"
                    )
                    continue
                    # Run pod5 inspect
                try:
...
"""
    ...
    run:
        ...
        combined.to_csv(
            output.master_report, sep="\t", index=False, compression="gzip"
        )
        ...
        with open(output.summary, "w") as f:
            ...
            f.write(
                f"ERROR files: {error_files} ({error_files/ total_files*100:.1f}%)\n\n"
            )
            if error_files > 0:
                f.write("=" * 80 + "\n")
            ...
            sample_summary = (
                ...
                .agg({"file": "count", "status": lambda x: (x == "OK").sum()})
                .rename(columns={"file": "total_files", "status": "ok_files"})
            )
            sample_summary["error_files"] = (
                sample_summary["total_files"] - sample_summary["ok_files"]
            )
            ...
            if len(ok_df) > 0:
                ok_df["num_reads"] = pd.to_numeric(
                    ok_df["num_reads"], errors="coerce"
                )
                f.write("\n" + "=" * 80 + "\n")
                f.write("Read Count Statistics (OK files only):\n")
                f.write("=" * 80 + "\n")
                f.write(
                    f"Total reads across all files: {ok_df['num_reads'].sum():,.0f}\n"
                )
                f.write(f"Mean reads per file: {ok_df['num_reads'].mean():,.0f}\n")
                f.write(
                    f"Median reads per file: {ok_df['num_reads'].median():,.0f}\n"
                )
