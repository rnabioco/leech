...
rule generate_run_summary:
    ...
    run:
        ...
        with open(output[0], "w") as f:
            ...
            if AA_PAIRS:
                f.write(f"**Comparison spec file:** `{config.get('comparison_spec_file')}`\n\n")
                ...
                for pair in AA_PAIRS:
                    if pair in AA_PAIR_TO_LABELS:
                        ...
                        f.write(f"- `{pair}`: {', '.join(labels1)} vs {', '.join(labels2)}\n")
                    else:
                        f.write(f"- `{pair}`\n")
            else:
            ...
            if config.get("use_grid_search"):
                ...
                f.write(f"- Early stopping patience: {config.get('early_stopping_patience', 5)}\n")

            f.write("\n---\n\n")
            f.write(f"*This summary was automatically generated by the Leech pipeline at {timestamp}*\n")
