...
4. **Format notebooks**: Run `uv run nbqa ruff format notebooks/` to auto-format Jupyter notebooks (if notebooks/ directory exists)
5. **Lint notebooks**: Run `uv run nbqa ruff notebooks/` to check notebooks for linting issues (if notebooks/ directory exists)
