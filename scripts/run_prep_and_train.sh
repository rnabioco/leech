#!/bin/bash
#SBATCH --job-name=leech_prep_train
#SBATCH --output=logs/prep_train_%j.out
#SBATCH --error=logs/prep_train_%j.err
...
# Orchestrator script: Run data preparation and model training
# This script runs the full workflow through training:
#   1. Prepare training chunks (all_prepare)
#   2. Train charged vs uncharged model (all_train)
#
# IMPORTANT: Submit this script from the project root directory with:
#   sbatch scripts/run_prep_and_train.sh
...
if [ -n "${SLURM_SUBMIT_DIR:-}" ]; then
    ...
    if [[ -f "${SCRIPT_DIR}/workflow/Snakefile" ]]; then
        ...
        echo "  cd /path/to/leech && sbatch scripts/run_prep_and_train.sh"
        exit 1
    fi
fi
...
if [[ ! -f "workflow/Snakefile" ]]; then
    ...
    echo "Please submit from project root: sbatch scripts/run_prep_and_train.sh"
    exit 1
fi

echo "=========================================="
echo "Leech Pipeline - Prepare and Train"
echo "=========================================="
...
echo "  2. Merge chunks from all samples for training"
...
echo "-------------------"
grep -A 1 "^  [a-z]" config/samples-alpine.yaml | grep -v "^--$" | grep "^  [a-z]" | sed 's/://g' | nl
echo ""
echo "Total: $(grep -A 1 "^  [a-z]" config/samples-alpine.yaml | grep -v "^--$" | grep "^  [a-z]" | wc -l) samples"
echo ""
